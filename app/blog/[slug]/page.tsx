...
export async function generateMetadata(props: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = primaryContent?.title || 'Legisland Blog';
  const description = primaryContent?.excerpt || 'Legal insights and intellectual property news from Legisland.';
  ...
  return {
    ...
    openGraph: {
      ...
      authors: post.author?.name ? [post.author.name] : ['Legisland'],
      images: [
        {
          url: imageUrl,
          width: 1200,
